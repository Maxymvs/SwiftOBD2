...
enum DTCProtocolFamily: Sendable, Hashable {
    ...
    case legacy
    /// Recognised but not parseable in this phase: 29-bit ISO 15765-4 (7/9), J1939 (A), the
    /// unmapped user protocols (B/C), and "no protocol established yet".
    ///
...
enum DTCResponseParser {
    ...
    static func parse(
        ...
        // Group frames by *raw* responder address — never by the lossy engine/transmission
        // classification, which is what collapses two non-engine modules onto one key.
        var order: [ECUAddress] = []
        var framesByResponder: [ECUAddress: [[UInt8]]] = [:]
        var damagedResponders: Set<ECUAddress> = []
        for line in sanitized.hexLines {
            switch frame(from: line, family: family) {
            case let .frame(address, payload):
                if framesByResponder[address] == nil {
                    framesByResponder[address] = []
                    order.append(address)
                }
                framesByResponder[address]?.append(payload)
            ...
            case let .damaged(address):
                ...
                damagedResponders.insert(address)
        ...
        for address in order {
            if damagedResponders.contains(address) {
                outcomes[address] = .malformed
                continue
            }
            guard let frames = framesByResponder[address] else { continue }
            let messages = family == .legacy
                ? legacyMessages(frames, service: service)
                : canMessages(frames)
    ...
    private static func frame(from line: String, family: DTCProtocolFamily) -> FrameExtraction {
        switch family {
        case .can11:
            // `7E8` + whole payload bytes. A complete three-nibble id *is* the responder
            // identity, even when nothing usable follows it: a header-only or truncated line is
            // that ECU's damage, never a line to drop.
            guard line.count >= 3, let id = UInt32(line.prefix(3), radix: 16) else {
                return .unrecoverable
            }
            let address = ECUAddress(raw: id)
            guard let payload = hexBytes(String(line.dropFirst(3))), !payload.isEmpty else {
    ...
    private static func legacyMessages(_ frames: [[UInt8]], service: DTCService) -> [RawMessage] {
        var messages: [RawMessage] = []
        var positive: [UInt8] = []
        var positiveIndex: Int?
        for frame in frames {
            if frame.first == service.positiveResponseByte {
